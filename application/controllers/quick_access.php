...
class Quick_access extends CI_Controller {
  ...
  public function index(){
    ...
    $data['items'] = $this->builtbyprime->explicit("SELECT IT.*, PPD.*, PPD.ID_ITEM_TASK AS ID_ITEM_TASK, (SELECT COUNT(*) FROM demon.TBL_DISCUSSION WHERE ID_ITEM_TASK = IT.ID) COMMENTS FROM demon.TBL_ITEM_TASK IT, demon.TBL_PROJECT_PLANNING_DETAIL PPD WHERE IT.ID = PPD.ID_ITEM_TASK AND PPD.START_WEEK < SYSDATE AND PPD.END_WEEK > SYSDATE");
