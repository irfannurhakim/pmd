...
class Home extends CI_Controller {
  ...
  public function topstat($sorting = 0, $year = null){
    ...
    $contractors = $this->builtbyprime->explicit("SELECT * FROM (SELECT v.affiliation name, p.id_vendor, COUNT(p.id_vendor) jml_proyek, SUM(p.budget) jml_nilai_proyek, v.proile_image_url FROM TBL_PROJECT p, TBL_USER v WHERE v.id = p.id_vendor and EXTRACT(YEAR FROM p.START_DATE) = '".$tahun."' GROUP BY p.id_vendor, v.affiliation ORDER BY ".$sortby." DESC) WHERE ROWNUM <= 7");
    $supervisors = $this->builtbyprime->explicit("SELECT * FROM (SELECT U.NAME, U.AFFILIATION, S.ID_USER, COUNT(S.ID_USER) JML_PROYEK, U.PROFILE_IMAGE_URL FROM TBL_SUPERVISOR_PROJECT S, TBL_USER U, TBL_PROJECT P WHERE U.ID = S.ID_USER AND P.ID = S.ID_PROJECT AND EXTRACT(YEAR FROM P.START_DATE) = '".$tahun."' GROUP BY U.NAME, S.ID_USER, U.AFFILIATION ORDER BY JML_PROYEK DESC) WHERE ROWNUM <= 7");
